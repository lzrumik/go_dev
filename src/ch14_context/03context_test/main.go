...
func task5(ctx context.Context) {
	...
	ctx5, cancel := context.WithTimeout(ctx, time.Second*8)
	defer cancel()
...
func main() {
	...
	task1 ctx
	task2 ctx     ctx2
	task3 ctx2    ctx3    5s 超时
	task4 ctx3    ctx4    3s 超时
	task5 ctx3    ctx5    6s 超时    因为ctx3 5s超时 所以5s时候被cancel
	task6 ctx5    ctx6    传入参数
	 */
