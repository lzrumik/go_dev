...
type FortuneLoanBakCopy struct {
...
func (t *FortuneLoanBakCopy) TableName() string {
	return "fortune_loan_bak_copy"
}

func init() {
	orm.RegisterModel(new(FortuneLoanBakCopy))
}

// AddFortuneLoanBakCopy insert a new FortuneLoanBakCopy into database and returns
// last inserted Id on success.
func AddFortuneLoanBakCopy(m *FortuneLoanBakCopy) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetFortuneLoanBakCopyById retrieves FortuneLoanBakCopy by Id. Returns error if
// Id doesn't exist
func GetFortuneLoanBakCopyById(id int) (v *FortuneLoanBakCopy, err error) {
	o := orm.NewOrm()
	v = &FortuneLoanBakCopy{Id: id}
...
// GetAllFortuneLoanBakCopy retrieves all FortuneLoanBakCopy matches certain condition. Returns empty list if
// no records exist
func GetAllFortuneLoanBakCopy(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(FortuneLoanBakCopy))
	// query k=v
	for k, v := range query {
...
// UpdateFortuneLoanBakCopy updates FortuneLoanBakCopy by Id and returns error if
// the record to be updated doesn't exist
func UpdateFortuneLoanBakCopyById(m *FortuneLoanBakCopy) (err error) {
	o := orm.NewOrm()
	v := FortuneLoanBakCopy{Id: m.Id}
...
// DeleteFortuneLoanBakCopy deletes FortuneLoanBakCopy by Id and returns error if
// the record to be deleted doesn't exist
func DeleteFortuneLoanBakCopy(id int) (err error) {
	o := orm.NewOrm()
	v := FortuneLoanBakCopy{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&FortuneLoanBakCopy{Id: id}); err == nil {
