...
type FortuneCustomersTools struct {
...
func (t *FortuneCustomersTools) TableName() string {
	return "fortune_customers_tools"
}

func init() {
	orm.RegisterModel(new(FortuneCustomersTools))
}

// AddFortuneCustomersTools insert a new FortuneCustomersTools into database and returns
// last inserted Id on success.
func AddFortuneCustomersTools(m *FortuneCustomersTools) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetFortuneCustomersToolsById retrieves FortuneCustomersTools by Id. Returns error if
// Id doesn't exist
func GetFortuneCustomersToolsById(id int) (v *FortuneCustomersTools, err error) {
	o := orm.NewOrm()
	v = &FortuneCustomersTools{Id: id}
...
// GetAllFortuneCustomersTools retrieves all FortuneCustomersTools matches certain condition. Returns empty list if
// no records exist
func GetAllFortuneCustomersTools(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(FortuneCustomersTools))
	// query k=v
	for k, v := range query {
...
// UpdateFortuneCustomersTools updates FortuneCustomersTools by Id and returns error if
// the record to be updated doesn't exist
func UpdateFortuneCustomersToolsById(m *FortuneCustomersTools) (err error) {
	o := orm.NewOrm()
	v := FortuneCustomersTools{Id: m.Id}
...
// DeleteFortuneCustomersTools deletes FortuneCustomersTools by Id and returns error if
// the record to be deleted doesn't exist
func DeleteFortuneCustomersTools(id int) (err error) {
	o := orm.NewOrm()
	v := FortuneCustomersTools{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&FortuneCustomersTools{Id: id}); err == nil {
