...
func (t *FortuneTemp) TableName() string {
	return "fortune_temp"
}

func init() {
	orm.RegisterModel(new(FortuneTemp))
}

// AddFortuneTemp insert a new FortuneTemp into database and returns
// last inserted Id on success.
func AddFortuneTemp(m *FortuneTemp) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetFortuneTempById retrieves FortuneTemp by Id. Returns error if
// Id doesn't exist
func GetFortuneTempById(id int) (v *FortuneTemp, err error) {
	o := orm.NewOrm()
	v = &FortuneTemp{Id: id}
...
// GetAllFortuneTemp retrieves all FortuneTemp matches certain condition. Returns empty list if
// no records exist
func GetAllFortuneTemp(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(FortuneTemp))
	// query k=v
	for k, v := range query {
...
// UpdateFortuneTemp updates FortuneTemp by Id and returns error if
// the record to be updated doesn't exist
func UpdateFortuneTempById(m *FortuneTemp) (err error) {
	o := orm.NewOrm()
	v := FortuneTemp{Id: m.Id}
...
// DeleteFortuneTemp deletes FortuneTemp by Id and returns error if
// the record to be deleted doesn't exist
func DeleteFortuneTemp(id int) (err error) {
	o := orm.NewOrm()
	v := FortuneTemp{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&FortuneTemp{Id: id}); err == nil {
