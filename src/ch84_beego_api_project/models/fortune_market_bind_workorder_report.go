...
type FortuneMarketBindWorkorderReport struct {
	...
	LastAllotStatus       int8      `orm:"column(last_allot_status)" description:"状态 0=未分配、1=已分配、2=已回收"`
...
func (t *FortuneMarketBindWorkorderReport) TableName() string {
	return "fortune_market_bind_workorder_report"
}

func init() {
	orm.RegisterModel(new(FortuneMarketBindWorkorderReport))
}

// AddFortuneMarketBindWorkorderReport insert a new FortuneMarketBindWorkorderReport into database and returns
// last inserted Id on success.
func AddFortuneMarketBindWorkorderReport(m *FortuneMarketBindWorkorderReport) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetFortuneMarketBindWorkorderReportById retrieves FortuneMarketBindWorkorderReport by Id. Returns error if
// Id doesn't exist
func GetFortuneMarketBindWorkorderReportById(id int) (v *FortuneMarketBindWorkorderReport, err error) {
	o := orm.NewOrm()
	v = &FortuneMarketBindWorkorderReport{Id: id}
...
// GetAllFortuneMarketBindWorkorderReport retrieves all FortuneMarketBindWorkorderReport matches certain condition. Returns empty list if
// no records exist
func GetAllFortuneMarketBindWorkorderReport(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(FortuneMarketBindWorkorderReport))
	// query k=v
	for k, v := range query {
	...
	var l []FortuneMarketBindWorkorderReport
	qs = qs.OrderBy(sortFields...)
...
// UpdateFortuneMarketBindWorkorderReport updates FortuneMarketBindWorkorderReport by Id and returns error if
// the record to be updated doesn't exist
func UpdateFortuneMarketBindWorkorderReportById(m *FortuneMarketBindWorkorderReport) (err error) {
	o := orm.NewOrm()
	v := FortuneMarketBindWorkorderReport{Id: m.Id}
...
// DeleteFortuneMarketBindWorkorderReport deletes FortuneMarketBindWorkorderReport by Id and returns error if
// the record to be deleted doesn't exist
func DeleteFortuneMarketBindWorkorderReport(id int) (err error) {
	o := orm.NewOrm()
	v := FortuneMarketBindWorkorderReport{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&FortuneMarketBindWorkorderReport{Id: id}); err == nil {
