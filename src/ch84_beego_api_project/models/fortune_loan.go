...
type FortuneLoan struct {
	Id                          int       `orm:"column(id);pk"`
	Company                     string    `orm:"column(company);size(20)" description:"所属公司"`
	LoanId                      string    `orm:"column(loan_id);size(70)" description:"名称(带编号)"`
	...
	Type                        int8      `orm:"column(type)" description:"项目类型: 0:类固定收益 1:美元固定收益 2:私募股权 3:海外置业 4:家庭保障•财富传承 5:私募证券 6:信托"`
...
func (t *FortuneLoan) TableName() string {
	return "fortune_loan"
}

func init() {
	orm.RegisterModel(new(FortuneLoan))
}

// AddFortuneLoan insert a new FortuneLoan into database and returns
// last inserted Id on success.
func AddFortuneLoan(m *FortuneLoan) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetFortuneLoanById retrieves FortuneLoan by Id. Returns error if
// Id doesn't exist
func GetFortuneLoanById(id int) (v *FortuneLoan, err error) {
	o := orm.NewOrm()
	v = &FortuneLoan{Id: id}
...
// GetAllFortuneLoan retrieves all FortuneLoan matches certain condition. Returns empty list if
// no records exist
func GetAllFortuneLoan(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(FortuneLoan))
	// query k=v
	for k, v := range query {
...
// UpdateFortuneLoan updates FortuneLoan by Id and returns error if
// the record to be updated doesn't exist
func UpdateFortuneLoanById(m *FortuneLoan) (err error) {
	o := orm.NewOrm()
	v := FortuneLoan{Id: m.Id}
...
// DeleteFortuneLoan deletes FortuneLoan by Id and returns error if
// the record to be deleted doesn't exist
func DeleteFortuneLoan(id int) (err error) {
	o := orm.NewOrm()
	v := FortuneLoan{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&FortuneLoan{Id: id}); err == nil {
