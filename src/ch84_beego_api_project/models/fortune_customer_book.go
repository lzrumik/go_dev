...
type FortuneCustomerBook struct {
...
func (t *FortuneCustomerBook) TableName() string {
	return "fortune_customer_book"
}

func init() {
	orm.RegisterModel(new(FortuneCustomerBook))
}

// AddFortuneCustomerBook insert a new FortuneCustomerBook into database and returns
// last inserted Id on success.
func AddFortuneCustomerBook(m *FortuneCustomerBook) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetFortuneCustomerBookById retrieves FortuneCustomerBook by Id. Returns error if
// Id doesn't exist
func GetFortuneCustomerBookById(id int) (v *FortuneCustomerBook, err error) {
	o := orm.NewOrm()
	v = &FortuneCustomerBook{Id: id}
...
// GetAllFortuneCustomerBook retrieves all FortuneCustomerBook matches certain condition. Returns empty list if
// no records exist
func GetAllFortuneCustomerBook(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(FortuneCustomerBook))
	// query k=v
	for k, v := range query {
...
// UpdateFortuneCustomerBook updates FortuneCustomerBook by Id and returns error if
// the record to be updated doesn't exist
func UpdateFortuneCustomerBookById(m *FortuneCustomerBook) (err error) {
	o := orm.NewOrm()
	v := FortuneCustomerBook{Id: m.Id}
...
// DeleteFortuneCustomerBook deletes FortuneCustomerBook by Id and returns error if
// the record to be deleted doesn't exist
func DeleteFortuneCustomerBook(id int) (err error) {
	o := orm.NewOrm()
	v := FortuneCustomerBook{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&FortuneCustomerBook{Id: id}); err == nil {
