...
type FortuneUser struct {
	...
	Code               string    `orm:"column(code);size(10);null" description:"公司ID"`
	Company            string    `orm:"column(company);size(20);null" description:"公司名称"`
	...
	GroupId            uint      `orm:"column(group_id)" description:"组ID"`
	...
	Department         uint8     `orm:"column(department)" description:"所属部门 1财富 2财富运营 3金融产品中心 4营销 5财务 6用研"`
	DataRole           string    `orm:"column(data_role);size(100)" description:"数据权限"`
...
func (t *FortuneUser) TableName() string {
	return "fortune_user"
}

func init() {
	orm.RegisterModel(new(FortuneUser))
}

// AddFortuneUser insert a new FortuneUser into database and returns
// last inserted Id on success.
func AddFortuneUser(m *FortuneUser) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetFortuneUserById retrieves FortuneUser by Id. Returns error if
// Id doesn't exist
func GetFortuneUserById(id int) (v *FortuneUser, err error) {
	o := orm.NewOrm()
	v = &FortuneUser{Id: id}
...
// GetAllFortuneUser retrieves all FortuneUser matches certain condition. Returns empty list if
// no records exist
func GetAllFortuneUser(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(FortuneUser))
	// query k=v
	for k, v := range query {
...
// UpdateFortuneUser updates FortuneUser by Id and returns error if
// the record to be updated doesn't exist
func UpdateFortuneUserById(m *FortuneUser) (err error) {
	o := orm.NewOrm()
	v := FortuneUser{Id: m.Id}
...
// DeleteFortuneUser deletes FortuneUser by Id and returns error if
// the record to be deleted doesn't exist
func DeleteFortuneUser(id int) (err error) {
	o := orm.NewOrm()
	v := FortuneUser{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&FortuneUser{Id: id}); err == nil {
